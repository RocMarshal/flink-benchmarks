...
package org.apache.flink.benchmark.flip370;
...
@OperationsPerInvocation(value = Flip370Benchmark.RECORDS_PER_INVOCATION)
public class Flip370Benchmark extends BenchmarkBase {
    ...
    public static void main(String[] args) throws RunnerException {
        Options options =
                new OptionsBuilder()
                        ...
                        .include(".*" + Flip370Benchmark.class.getCanonicalName() + ".*")
                        .build();
    ...
    public void throughput(InputBenchmarkFlinkEnvironmentContext context) throws Exception {
        ...
        for (int i = 1; i < parallelisms.length; i++) {
            index = index.map(new MultiplyByTwo()).setParallelism(allParallelismsOfVerticesSame ? parallelisms[parallelisms.length-1] : parallelisms[i]);
        }
